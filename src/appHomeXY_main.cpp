    ...
    const std::string& sourceModule)
{
    ...
    if (socketFd < 0) {
        std::fprintf(stderr, "appHomeXY: socket creation failed\n");
        return false;
    }
    ...
    if (::inet_pton(AF_INET, ip.c_str(), &targetAddress.sin_addr) != 1) {
        std::fprintf(stderr, "appHomeXY: invalid target IP '%s'\n", ip.c_str());
        ::close(socketFd);
        return false;
    }

    nlohmann::json payloadJson;
    payloadJson["action"] = "homeXYAB";
    ...
    if (bytesSent < 0) {
        std::fprintf(stderr, "appHomeXY: send failed\n");
        return false;
    }

    std::fprintf(
        stderr,
        "appHomeXY: sent homeXYAB command to module '%s' (%s:%u)\n",
...
int main(int argc, char** argv)
{
    ...
    std::string destinationModule = "moveXY";
    if (argc > 2 && argv[2] != nullptr) {
        destinationModule = argv[2];
    }

    const std::string sourceModule = "appHomeXY";
    ...
    if (!configLoader.load(configPath, appConfig, errorMessage)) {
        std::fprintf(stderr, "appHomeXY: config load failed: %s\n", errorMessage.c_str());
        return 1;
    }
