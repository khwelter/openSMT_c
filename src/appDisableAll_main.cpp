    ...
    const std::string& destinationModule)
{
    ...
    if (socketFd < 0) {
        std::fprintf(stderr, "appDisableAll: socket creation failed\n");
        return false;
    }
    ...
    if (::inet_pton(AF_INET, ip.c_str(), &targetAddress.sin_addr) != 1) {
        std::fprintf(stderr, "appDisableAll: invalid target IP '%s'\n", ip.c_str());
        ::close(socketFd);
        return false;
    }

    nlohmann::json payloadJson;
    payloadJson["action"] = "disableAllSteppers";
    ...
    frameJson["sourceModule"] = "appDisableAll";
    ...
    if (bytesSent < 0) {
        std::fprintf(stderr, "appDisableAll: send failed for module '%s'\n", destinationModule.c_str());
        return false;
    }

    std::fprintf(stderr, "appDisableAll: sent disableAllSteppers to module '%s'\n", destinationModule.c_str());
    return true;
}
...
int main()
{
    ...
    if (!configLoader.load("config/config.json", appConfig, errorMessage)) {
        std::fprintf(stderr, "appDisableAll: config load failed: %s\n", errorMessage.c_str());
        return 1;
    }

    // One representative driver per physical Marlin board.
    const std::vector<std::string> destinations = {"moveXY", "moveZ1", "moveZ3"};

    bool allOk = true;
    for (const auto& destination : destinations) {
        if (!sendDisable(appConfig.communication.listenIp, appConfig.communication.listenPort, destination)) {
            allOk = false;
        }
    }
